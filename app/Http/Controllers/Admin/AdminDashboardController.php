...
class AdminDashboardController extends Controller
{
    ...
    public function csmCustomizationNew(){
        return view('admin.customization.new');
    }
    
    public function csmCustomizationAll(){
        $data = DB::select("SELECT * FROM `proof_of_work` ORDER BY date DESC");
        return view('admin.customization.all', ['data' => $data]);
    }
    
    public function csmCustomizationUpdate($id){
        
        $data = DB::select("SELECT * FROM `proof_of_work` WHERE `id`='$id'");
        if(empty($data)){
            return redirect()->route('admin.csmCustomizationAll')->withErrors(['not_found' => 'data not found!']);
            echo "not exist";
        }else{
            return view('admin.customization.update', ['data' => $data]);
        }
    }
    
    public function csmCustomizationNewManage(Request $req){
      
        $this->validate($req, [
            'date' => 'required|unique:proof_of_work,date',
            'algo' => 'nullable',
            'workers' => 'nullable',
            'hashrate' => 'nullable',
            'network' => 'nullable',
        ...
        $insert = DB::table('proof_of_work')->insert([
                'date' => $inputs['date'],
                'algo' => $inputs['algo'],
                'active_workers' => $inputs['workers'],
                'hashrate' => $inputs['hashrate'],
                'network' => $inputs['network'],
                'btc' => $inputs['btc'],
                'usd' => $inputs['usd'],
                'difficulty' => $inputs['difficulty'],
                'nextdifficulty' => $inputs['nextdifficulty'],
                'datetonext' => $inputs['datetonext'],
                
            ]);
        
        if($insert){
            return redirect()->route('admin.csmCustomizationAll')->with('success', 'Insert successfully');
        }
        
        
    }
    
    public function csmCustomizationUpdateManage(Request $req){
        
        $id = $req->all()['id'];
        
        $this->validate($req, [
            'id' => 'required',
            'date' => 'required|unique:proof_of_work,date,'.$id,
            'algo' => 'nullable',
            'workers' => 'nullable',
            'hashrate' => 'nullable',
            'network' => 'nullable',
        ...
        $update = DB::table('proof_of_work')->where('id', $id)->update([
                'date' => $inputs['date'],
                'algo' => $inputs['algo'],
                'active_workers' => $inputs['workers'],
                'hashrate' => $inputs['hashrate'],
        ...
        if($update){
            return redirect()->route('admin.csmCustomizationAll')->with('success', 'Updated successfully');
        }
        
    }
    
    public function csmCustomizationDeleteManage($id){
        
        $deleted = DB::table('proof_of_work')->where('id', $id)->delete();
