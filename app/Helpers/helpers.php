...
if (!function_exists('binary_num')) {
    /**
     * @return string
     * @version 1.0.0
     * @since 1.3.0
     */
    function binary_num($len = 8, $num = 0)
    {
        $bnum = '';
        $n1 = ($num > 0) ? 1 : 0;
        $n2 = ($n1 == 1) ? 0 : 1;
...
if (!function_exists('str_comln')) {
    /**
     * @return boolean
     * @version 1.0.0
     * @since 1.0
     */
    function str_comln($string, $len = 30)
    {
        return Str::length($string) >= $len;
    }
}
...
if (!function_exists('sample_num')) {
    /**
     * @return string
     * @version 1.0.0
     * @since 1.3.0
     */
    function sample_num($start = 0, $count = null, $str = null)
    {
        $output = '';
        $count = $count ?? 11;
        $num = $count - $start + 2;
...
if (!function_exists('sys_settings')) {
    ...
    function sys_settings($key, $default = null)
    {
        ...
        $value = ($key == 'prefix') ? Cache::get('site_prefix') : $value;
...
if (!function_exists('gui_body')) {
    ...
    function gui_body()
    {
        $prefix = Cache::get('site_prefix');
        return empty($prefix) ? cipher('body') : $prefix;
    }
}
