...
<div class="nk-plan-item">
    ...
    <div class="nk-plan-info w-max-275px">
       <div class="nk-plan-name">{{ucfirst( data_get($scheme, 'name')) }}</div>

        <div class="nk-plan-desc">{{ __('Lease Price:') }} <span class="amount">{{ data_get($plan, 'amount')}} <img src="/images/bitcoin.svg"></span></div>
       <!-- <div class="nk-plan-name">{{ __('Lease Price') }}</div>-->
    </div>
    <div class="nk-plan-term">
        <div class="nk-plan-start nk-plan-order">
            <span class="nk-plan-label text-soft">{{ __('Lease Start Date') }}</span>
            <span class="nk-plan-value date">{{ show_date(data_get($plan, 'term_start'), true) }}</span>
        </div>
        <div class="nk-plan-end nk-plan-order">
            <span class="nk-plan-label text-soft">{{ __('Lease End Date') }}</span>
