...
.col-md-6.col-lg-4.offset-lg-2.totla {
    margin-left: -18.2%;
    margin-top: 0px;
}
.nk-plan-details.dsfsdfsdf {
    position: relative;
    display: flex;
    ...
    <div class="nk-content-body">
        <div class="nk-block-head">
            <!--<div class="nk-plan-details dsfsdfsdf">
              {{__(':Name | Investment Plan', ['name' => __(data_get($invest, 'summary_title_alter'))])}}
             </div>-->
            <div class="nk-block-head-sub"><a href="{{ route('user.investment.dashboard') }}" class="text-soft back-to"><em class="icon ni ni-arrow-left"> </em><span>{{ __("My Devices") }}</span></a></div>
            <div class="nk-block-between-md g-4">
                <div class="nk-block-head-content">
                   <!-- <h3 class="nk-block-title fw-normal">{{ data_get($invest, 'summary_title_alter') }}</h3>-->
                   <h3 class="nk-block-title fw-normal">{{ __('News') }}</h3>
                ...
                <div class="nk-block-head-content">
                    <ul class="nk-block-tools gx-3">
                        ...
                        <li class="order-md-last">
                            <button type="button" class="btn btn-danger iv-plnot" data-action="cancelled" data-confirm="yes">
        ...
        <div class="nk-block">
            ...
            <div class="card card-bordered">
                <div class="card-inner">
                    <div class="row gy-gs">
                            ...
                            <div class="nk-wgacc">
                                <div class="nk-wgacc-group flex-lg-nowrap gx-4">
                                    <div class="nk-wgacc-sub">
                                         <h5 class="nk-wgpg-title">Daily Proceeds </h5>
                                        <div class="nk-wgacc-amount">
                                            <div class="number">{{ $csm_investment_history['today'] }} <img class="imagess" src="https://upload.wikimedia.org/wikipedia/commons/4/46/Bitcoin.svg" alt="Girl in a jacket"></div>
                                        </div>
                        ...
                        <div class="col-md-6 col-lg-4 offset-lg-2 totla">
                            <div class="nk-wgacc pl-md-3">
                                <div class="nk-wgacc-group flex-lg-nowrap gx-4">
                                    <div class="nk-wgacc-sub">
                                        <h5 class="nk-wgpg-title">Total Proceeds </h5>
                                        <div class="nk-wgacc-amount">
                                            <div class="number">
                                                 {{ $csm_investment_history['total'] }} <img class="imagess" src="https://upload.wikimedia.org/wikipedia/commons/4/46/Bitcoin.svg" alt="Girl in a jacket"></span>
                                            </div>
                                        </div>
                ...
                <div class="nk-plan-details">
                    <ul class="nk-wgacc-list">
                       <!-- <li>
                        ...
                        <li>
                            <div class="sub-text">{{ __('Uptime') }}</div>
                            <div class="lead-text">{{ data_get($invest, 'term_text_alter') }}</div>
                        </li>
                        <li>
                            <div class="sub-text">{{ __('Uptime start') }}</div>
                            <div class="lead-text">
                        ...
                        <li>
                            <div class="sub-text">{{ __('Uptime end') }}</div>
                            <div class="lead-text">
                                @if($invest->status == 'pending' || $invest->status == 'cancelled')
                                    -
                                @else
         ...
         <!--<div class="container bg-white text-center my-5 border rounded">
            <div class="row p-5">
                <div class="col-md-6">
                    <h6>Daily</h6>
                    <p>{{ $csm_investment_history['today'] }} {{ $currency }}</p>
                </div>
               <div class="col-md-3">
                    <h6>Weekly</h6>
                    <p>{{ $csm_investment_history['weekly'] }} {{ $currency }}</p>
                </div>
               <div class="col-md-3">
                    <h6>Monthly</h6>
                    <p>{{ $csm_investment_history['monthly'] }} {{ $currency }}</p>
                </div>
                <div class="col-md-6">
                    <h6>Total</h6>
                    <p>
                        {{ $csm_investment_history['total'] }} {{ $currency }}
                        <br>
        ...
        <!--        <div class="col-md-3">-->
        <!--            <h6>Daily</h6>-->
        <!--            <p>{{ ($exchange_rates['automatic']['BTC'] * $invest->received) / ($invest['scheme']['term']*30)}}</p>-->
        <!--        </div>-->
        <!--        <div class="col-md-3">-->
        <!--            <h6>Weekly</h6>-->
        <!--            <p>{{ ($exchange_rates['automatic']['BTC'] * $invest->received) / ($invest['scheme']['term'] * 7) }}</p>-->
        <!--        </div>-->
        <!--        <div class="col-md-3">-->
        <!--            <h6>Monthly</h6>-->
        <!--            <p>{{ ($exchange_rates['automatic']['BTC'] * $invest->received) / $invest['scheme']['term'] }}</p>-->
        <!--        </div>-->
        <!--        <div class="col-md-3">-->
        <!--            <h6>Total</h6>-->
        <!--            <p>{{ $exchange_rates['automatic']['BTC'] * $invest->received }}</p>-->
        <!--        </div>-->
        <!--    </div>-->
        <!--</div>-->
                    ...
                    <div class="card card-bordered h-100">
                        <div class="card-inner justify-center text-center h-100">
                            <div class="nk-wgpg">
                                ...
                                <div class="nk-wgpg-graph">
                                    <input type="text" class="knob-half" value="{{ $csm_investment_history['total_percentage'] }}" data-fgColor="#6576ff" data-bgColor="#d9e5f7" data-thickness=".06" data-width="300" data-height="155" data-displayInput="false">
                                    <div class="nk-wgpg-graph-result">
                                        <div class="text-lead">{{ $csm_investment_history['total_percentage'] }}%</div>
                                        <div class="text-sub">{{ data_get($invest, 'rate_text') }} / {{ strtolower(data_get($invest, 'period_text')) }}</div>
                                    </div>
                                    <div class="nk-wgpg-graph-minmax"><span>{{ $csm_investment_history['total'] . ' ' . $currency }}</span><span>{{ money(data_get($invest, 'total'), $currency) }}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-bordered h-100">
                        <div class="card-inner justify-center text-center h-100">
                            <div class="nk-wgpg">
                                ...
                                <div class="nk-wgpg-graph sm">
                                    <input type="text" class="knob-half" value="{{ $csm_investment_history['profit_percentage'] }}" data-fgColor="#33d895" data-bgColor="#d9e5f7" data-thickness=".07" data-width="240" data-height="125" data-displayInput="false">
                                    <div class="nk-wgpg-graph-result">
                                    ...
                                    <div class="nk-wgpg-graph-minmax"><span>{{ $csm_investment_history['profit'] . ' ' . $currency }}</span><span>{{ money(data_get($invest, 'profit'), $currency) }}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-bordered h-100">
                        <div class="card-inner justify-center text-center h-100">
                            <div class="nk-wgpg">
                                ...
                                <div class="nk-wgpg-graph sm">
                                    <input type="text" class="knob-half" value="{{ $csm_investment_history['remaining_days_percentage'] }}" data-fgColor="#816bff" data-bgColor="#d9e5f7" data-thickness=".07" data-width="240" data-height="125" data-displayInput="false">
                                    <div class="nk-wgpg-graph-result">
                                        <div class="text-lead sm">{{ $csm_investment_history['remaining_days'] }}</div>
                                        <div class="text-sub">{{ __('remain to adjust') }}</div>
                                    </div>
                                    <div class="nk-wgpg-graph-minmax"><span>{{ $csm_investment_history['ongoing_day'] }} Times</span><span>{{ __(':count Times', ['count' => data_get($invest, 'term_total')]) }}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
